...
public class EncerrarRoteiroUseCase {
    ...
    public EncerrarRoteiroUseCase(PedidoGateway pedidoGateway, EncerramentoPedidoStrategyFactory strategyFactory) {
        this.pedidoGateway = pedidoGateway; this.strategyFactory = strategyFactory;
    }
    ...
    public List<EncerramentoPedidoOutput> executar(EncerrarRoteiroInput input) {
        ...
        Map<String, Pedido> pedidosPorId = pedidos.stream().collect(Collectors.toMap(Pedido::getId, Function.identity()));
        ...
        for (EncerrarPedidoItemInput item : input.itens()) {
            ...
            if (pedido == null) {
                resultados.add(new EncerramentoPedidoOutput(item.pedidoEntregaId(), true, "Pedido não encontrado na base de dados.", null, null, null));
                continue;
            }
            ...
            try {
                ...
                EncerramentoPedidoOutput resultado = strategy.processar(pedido, input.roteiroId(), item.novoStatusOcorrencia(), item.motivoInsucesso(), input.usuarioId());

                resultados.add(resultado);

            } catch (IllegalArgumentException ex) {
                resultados.add(new EncerramentoPedidoOutput(pedido.getId(), true, "Nenhuma estratégia encontrada para o encerramento: " + ex.getMessage(), null, null, null));
            }
        }
