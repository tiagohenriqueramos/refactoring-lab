...
public final class EncerramentoRoteiroControllerMapper {
    ...
    public static EncerrarRoteiroInput toUseCaseInput(String roteiroId, String usuarioId, List<EncerrarPedidoRoteiroRequest> request) {
        List<EncerrarPedidoItemInput> itens = request.stream().map(item -> new EncerrarPedidoItemInput(item.pedidoEntregaId(), item.novoStatus(), item.novoStatusOcorrencia(), item.motivoInsucesso())).toList();
    ...
    public static List<EncerramentoPedidoRoteiroResponse> toResponseList(List<EncerramentoPedidoOutput> output) {
        return output.stream().map(item -> new EncerramentoPedidoRoteiroResponse(item.pedidoId(), item.possuiErro(), item.mensagem(), item.pedidoRemoverRoteiroId(), item.pedidosRemoverRoteiroId(), item.etapaAdicionarId())).toList();
    }
}
